 ...
 */
template <typename T>
class solver::AugRie: public WavePropagation<T> {
...
public:
  ...
  AugRie(
    ...
    std::cout
      << "  *** solver::AugRie created" << std::endl
...
private:
  ...
  void determineWetDryState() {
    ...
    // dry cell lies higher than the wet cell
      ...
      computeMiddleState(hRight, hRight, -uRight, uRight, -huRight, huRight, maxNumberOfNewtonIterations);
      ...
      //        \cite[ch. 6.8.2]{george2006finite})
      //        \cite[ch. 5.2]{george2008augmented}
      computeMiddleState(hLeft, hLeft, uLeft, -uLeft, huLeft, -huLeft, maxNumberOfNewtonIterations);
   ...
   */
  inline void computeWaveDecomposition(
    T o_fWaves[3][2],
    T o_waveSpeeds[3]
#if CONFIG_TSUNAMI_AUGMENTED_RIEMANN_EIGEN_COEFFICIENTS
    ,
    ...
    if (wetDryState != WetDryWall && wetDryState != DryWetWall) { // case WDW and DWW was computed in
                                                                  // determineWetDryState already
      computeMiddleState(hLeft, hRight, uLeft, uRight, huLeft, huRight);
    }
    ...
    if ((riemannStructure == ShockRarefaction || riemannStructure == RarefactionShock || riemannStructure == RarefactionRarefaction) && hMiddle > dryTol) { // limit to ensure non-negative depth
      ...
      if ((riemannStructure == RarefactionShock || riemannStructure == RarefactionRarefaction) && eigenValues[0] * middleStateSpeeds[0] < (T).0) { // transsonic rarefaction, first wave family
        rareBarrier[0] = (T).2;
      } else if ((riemannStructure == ShockRarefaction || riemannStructure == RarefactionRarefaction) && eigenValues[2] * middleStateSpeeds[1] < (T)0.) { // transsonic rarefaction, second wave family
        rareBarrier[0] = (T).2;
      }
    ...
    rightHandSide[2] = huRight * uRight + (T).5 * g * hRight * hRight - (huLeft * uLeft + (T).5 * g * hLeft * hLeft);
  ...
  inline void computeMiddleState(
    ...
    if (riemannStructure == RarefactionRarefaction) {
      ...
      hMiddle = std::max(
        (T)0., i_uLeft - i_uRight + (T)2. * (l_sqrt_g_hLeft + l_sqrt_g_hRight)
    ...
    if (riemannStructure == ShockRarefaction || riemannStructure == RarefactionShock) {
      ...
      for (int i = 0; i < i_maxNumberOfNewtonIterations; i++) {
        ...
        T derivativePhi = sqrtTermHMin - (T).25 * g * (hMiddle - hMin) / (hMiddle * hMiddle * sqrtTermHMin)
   ...
   */
  inline RiemannStructure determineRiemannStructure(
    const T& i_hLeft, const T& i_hRight, const T& i_uLeft, const T& i_uRight
  ) const {
   ...
   * @param o_x solution
   */
  static inline void solveLinearEquation(const T i_matrix[3][3], const T i_b[3], T o_x[3]) {
    //#if 1
    // compute inverse of 3x3 matrix
    const T m[3][3] = {
      {(i_matrix[1][1] * i_matrix[2][2] - i_matrix[1][2] * i_matrix[2][1]),
       -(i_matrix[0][1] * i_matrix[2][2] - i_matrix[0][2] * i_matrix[2][1]),
       (i_matrix[0][1] * i_matrix[1][2] - i_matrix[0][2] * i_matrix[1][1])},
      {-(i_matrix[1][0] * i_matrix[2][2] - i_matrix[1][2] * i_matrix[2][0]),
       (i_matrix[0][0] * i_matrix[2][2] - i_matrix[0][2] * i_matrix[2][0]),
       -(i_matrix[0][0] * i_matrix[1][2] - i_matrix[0][2] * i_matrix[1][0])},
      {(i_matrix[1][0] * i_matrix[2][1] - i_matrix[1][1] * i_matrix[2][0]),
       -(i_matrix[0][0] * i_matrix[2][1] - i_matrix[0][1] * i_matrix[2][0]),
