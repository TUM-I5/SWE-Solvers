...
public:
	 ...
	 * @param dryTol "dry tolerance": if the water height falls below dryTol, wall boundary conditions are applied (default value is 0.01)
...
{
	T hLeft = i_hLeft;			T hRight = i_hRight;
	T uLeft = (T)0;	T uRight = (T)0;
	T huLeft = i_huLeft;			T huRight = i_huRight;
	T bLeft = i_bLeft;			T bRight = i_bRight;
 ...
 **************************************************************************************/
	...
	if (hLeft >= dryTol) {
		...
		if (hRight >= dryTol) {
			...
			#ifdef VECTORIZE // Vectorization fails with branching return statement. Let computation resume with dummy values instead.
			hLeft = hRight = dryTol;
			uLeft = uRight = (T)0;
			huLeft = huRight = (T)0;
			bLeft = bRight = (T)0;
 ...
 **************************************************************************************/
		...
		const T characteristicSpeeds[2] = {
			uLeft - sqrt_g_hLeft,
		...
		const T hLLMiddleHeight = MAX((huLeft - huRight + extEinfeldtSpeeds[1] * hRight - extEinfeldtSpeeds[0] * hLeft) / (extEinfeldtSpeeds[1] - extEinfeldtSpeeds[0]), (T)0);

        //define eigenvalues
		const T eigenValues[3] = {
			extEinfeldtSpeeds[0],
			(T)0.5 * (extEinfeldtSpeeds[0] + extEinfeldtSpeeds[1]),
			extEinfeldtSpeeds[1]
		};
		...
		if (eigenValues[0] < -zeroTol && eigenValues[2] > zeroTol) {
			//subsonic
			steadyStateWave[0] = MAX(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[0]);
			steadyStateWave[0] = MIN(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[2]);
		} else if (eigenValues[0] > zeroTol) {
			//supersonic right TODO: motivation?
			steadyStateWave[0] = MAX(steadyStateWave[0], -hLeft);
			steadyStateWave[0] = MIN(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[0]);
		} else if (eigenValues[2] < -zeroTol) {
			//supersonic left TODO: motivation?
			steadyStateWave[0] = MAX(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[2]);
